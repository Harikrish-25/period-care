...
import { 
  BarChart3, Package, Users, TrendingUp, CheckCircle, XCircle, Clock,
  Edit, Trash2, Plus, Save, X, Settings, MessageSquare, Star
} from 'lucide-react';
import { ordersService, kitsService, cmsService } from '../services/api';
...
const AdminDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      const [ordersData, kitsData, benefitsData, testimonialsData] = await Promise.all([
  ...
  const handleKitSave = async (kit: Kit) => {
    try {
      if (editingKit) {
        const updatedKit = await kitsService.updateKit(kit.id, kit);
        setKits(prev => prev.map(k => k.id === kit.id ? updatedKit : k));
      } else {
        const newKit = await kitsService.createKit(kit);
  ...
  const handleKitDelete = async (kitId: string) => {
    try {
      await kitsService.deleteKit(kitId);
  ...
  const handleBenefitSave = async (benefit: Benefit) => {
    try {
      if (editingBenefit) {
        const updatedBenefit = await cmsService.updateBenefit(benefit.id, benefit);
        setBenefits(prev => prev.map(b => b.id === benefit.id ? updatedBenefit : b));
      } else {
        const newBenefit = await cmsService.createBenefit(benefit);
  ...
  const handleTestimonialSave = async (testimonial: Testimonial) => {
    try {
      if (editingTestimonial) {
        const updatedTestimonial = await cmsService.updateTestimonial(testimonial.id, testimonial);
        setTestimonials(prev => prev.map(t => t.id === testimonial.id ? updatedTestimonial : t));
      } else {
        const newTestimonial = await cmsService.createTestimonial(testimonial);
  ...
  const handleTestimonialDelete = async (testimonialId: string) => {
    try {
      await cmsService.deleteTestimonial(testimonialId);
